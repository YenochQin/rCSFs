...
> **注意：以上是上游 GRASP `rcsfinteract90` 的约束，不等于本仓库的约束。**
> 本仓库的 `rcsfs.select_interacting_csfs` 有意放宽了第 3 条：参考文件的 peel
> 轨道列表只需是候选文件 peel 轨道列表的**前缀**（顺序一致），不要求完全相同。
> 这样同一个参考空间可以直接用于追加了更多关联轨道的候选空间。输出沿用候选文件
> 的头部（含追加的轨道），追加的轨道也参与占据数比较；块数与各块 J/P 仍须一致。
> 另外本仓库要求两个输入必须是不同的文件（含符号链接与硬链接），输出也不得与任一
> 输入指向同一文件。详见 README 的 `rcsfs interacting` 一节。
...
`rCSFs` 现已提供第一阶段的 `select_interacting_csfs` Python API 和
`interacting` CLI（默认输出 `rcsf.out`，默认 8 个 worker）。当前
`structural_upper_bound` 方法在对应 `J^P` 块中
执行电子数守恒、每壳层 `|Δq|<=2`、差异壳层数不超过 4、
`IDQ=sum|Δq|<=4` 的占据预筛，并把 `exact=False` 写入统计结果。它保持
MR/候选顺序、跳过规范整数表示完全相同的 MR 项，并保证不同 worker 数产生
相同输出；但当前会把完整输入解析到内存，并不等价于原程序的角向判定。

要继续实现精确 Rust 复现，仅比较描述符的占据差异还不够：
...
前两层已由当前结构上界方法实现；后两层仍需要完整的 GRASP 角动量语义，
不能由当前固定长度 ML 描述符无损替代。DC 与 DCB 参数目前都使用相同占据
上界，只作为后续精确内核的显式物理模式保留。
