...
impl CompleteCsfFile {
    ...
    pub fn parse_reader(mut reader: impl BufRead) -> Result<Self> {
        // Keep line terminators visible: `BufRead::lines` would silently normalize
        // CRLF and a missing final LF, violating the lossless contract.
        let mut lines = std::iter::from_fn(move || {
            let mut line = String::new();
            match reader.read_line(&mut line) {
                Ok(0) => None,
                Ok(_) => Some((|| -> Result<String> {
                    ensure!(line.ends_with('\n'), "CSF lines must end with LF");
                    line.pop();
                    ensure!(
                        !line.ends_with('\r'),
                        "CRLF is not canonical; CSF lines must end with LF only"
                    );
                    Ok(line)
                })()),
    ...
    fn format_record(&self, record: &CsfRecord) -> Result<(String, String, String)> {
        format_record(
            record,
            &self.subshells,
            self.occupied(record)?,
            self.couplings(record)?,
        )
    }
...
fn parse_record(
    ...
    let (canonical1, canonical2, canonical3) = format_record(
        &record,
        subshells,
        checked_slice(
            occupied_arena,
    ...
    for ((line_number, original), canonical) in
        lines.iter().zip([canonical1, canonical2, canonical3])
    {
        ensure!(
            original == &canonical,
            "line {line_number}: non-canonical CSF record text"
        );
    }
    Ok(record)
}
