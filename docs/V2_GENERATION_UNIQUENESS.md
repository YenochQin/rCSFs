...
状态：结论已给出（2026-09-22）。**生产路径在 P6b 落地前仍使用精确去重**；本文件只决定后续架构，
不改变现有行为。结论适用于 rCSFs 自身的内部生成路径，不适用于外部 CSF 文本输入。
...
- **P6b：具备启动门槛。** 证明与回归都在本文件与 `tests/p6a_*` 中；实施时应当
  保留精确去重实现与开关，仅对满足上述不变量的内部构造启用快速路径，并在发现不变量
  不满足时自动回退到精确路径（计划 P6b 的既有要求）。
