...
pub(crate) enum ArtifactKind {
    ...
    CsfParquet,
    Descriptor,
    /// The header and the descriptor sidecar, which share one allowance.
    Metadata,
}
...
impl CapacityEstimate {
    ...
    pub(crate) fn artifact_bytes(&self, kind: ArtifactKind) -> u64 {
        match kind {
            ...
            ArtifactKind::Metadata => METADATA_BYTES,
        }
    }
}
...
fn scaled_by_margin(value: u64) -> Result<u64> {
    with_margin(value)
}
...
pub(crate) enum SpacePolicy {
    ...
    ///
    /// `allow_unchecked` is the explicit way to accept a volume the platform
    /// cannot measure; without it an unknown value is treated as a failure
    /// instead of as a silent pass.
...
pub(crate) fn check_space(
    ...
    for (path, role) in entries {
        ...
        match role {
            SpaceRole::Scratch => group.scratch = group.scratch.max(estimate.required_scratch_bytes),
            ...
            SpaceRole::Published { bytes } => {
                group.published = group
                    .published
                    .checked_add(scaled_by_margin(*bytes)?)
...
mod tests {
    ...
    #[test]
    fn an_insufficient_volume_stops_a_run_but_is_reported_by_an_estimate() {
        // A requirement larger than any real volume, so the check fails on the
        // free-space comparison rather than on an unmeasurable path.
        let mut estimate = estimate_capacity(56, 1_000_000, 2).unwrap();
        estimate.required_scratch_bytes = u64::MAX / 2;
        let entries = [(
            std::env::temp_dir().join("rcsfs-capacity-test-scratch"),
            SpaceRole::Scratch,
        )];
        ...
        .expect_err("a run must not start on a volume that cannot hold it");
        assert!(error.to_string().contains("not enough free space at"));
        // The opt-out does not make the volume big enough, it only stops the
        // refusal, and the size is still reported as insufficient.
        let reported = check_space(
            &estimate,
            &entries,
            SpacePolicy::Require {
    ...
    fn requirements_on_one_volume_are_added_together() {
        ...
        let publication = estimate.required_output_bytes
            + with_margin(estimate.staged_output_bytes).unwrap();
    ...
    fn an_unmeasurable_or_impossible_requirement_is_refused_by_a_run() {
        ...
        let error = check_space(&estimate, &entries, SpacePolicy::Require { allow_unchecked: false })
            .expect_err("an unmeasurable volume must not pass silently");
        assert!(error.to_string().contains("cannot check free space"));
        let accepted =
            check_space(&estimate, &entries, SpacePolicy::Require { allow_unchecked: true }).unwrap();
