...
- `[csfsgenerate]` 可用有序的 `[[csfsgenerate.lists]]` 提交多个独立参考列表；
  核与输出路径共享，各列表必须明确写出参考组态、活动空间、`2J` 范围和激发数。
  旧的单列表平铺配置继续可用，混用两种写法会报错。
- 多列表统一 Peel 列、逐组生成，并自动使用完整 V2 记录的精确去重；
  无描述符请求也使用磁盘流水线。估算与实际生成按同一组数、列宽和去重策略计算，
  闭核或电子总数不一致时拒绝合并。

### P4 并行 descriptor 编码与 CLI 安全发布（2026-09-23）

- Parquet descriptor 按列在 Rayon 工作线程编码，完成的列块由单一写入器按顺序
  追加到一个文件；每次只有一个行组在途，并在分配前为编码缓冲和列块预留内存。
  两遍参考差分检查逻辑 V2 行、schema、元数据以及 CSF 文本和 Parquet 行。
- 干净提交 `c91e51e` 的 8 线程测量：B1/B2 descriptor encode 墙钟
  1.526→0.395 / 0.727→0.176 秒，端到端 4.075→3.317 / 1.512→0.979 秒；
  新物理摘要、RSS 与资源峰值见[登记报告](benchmarks/v2_disk_generation_parallel_descriptor_20260923.md)。
- CLI 对每个完整产物用无覆盖 hard link 原子显现；跨卷先复制至目标旁私有临时文件。
  这不是五文件事务；后续发布失败时已完成的文件保留，错误报告列明成功路径与失败目标。
  维护测试覆盖竞争写入与跨卷回退。

### P4 评审修正与七相位复测（2026-09-23）
...
- 上述修正已在干净提交 `fbcf884` 上按 clean-source 门禁复测：尾部 B1/B2 由两遍路径的
  8.536/2.575 秒降到 3.422/1.209 秒（2.49×/2.13×），累计相对 `7ad18b1` 为
  3.53×/3.19×。descriptor encode 仍占尾部 45%/60%，支持继续投入并行列块编码。
...
- 补齐两项登记缺口：每个组合的**独立进程 RSS**（codec 实验）与 B1/B2 **四个发布产物的
  SHA-256 内容摘要**（P6b 差分）。codec 与 dedup 四份 JSON 均在干净树 `47e04ea` 重测，
  计时与 `bbcd714`/`3588df1` 轮在同机同扩展上一致（扩展字节相同：其间提交只改了文档、
  harness 与测试）。
- **内容差分证据**：B1/B2 上两种去重策略、两种 codec 的全部运行发布完全相同的字节，
  四个产物的 SHA-256 逐一相同并登记在 [dedup 报告](benchmarks/v2_disk_generation_dedup_20260922.md)
  的表格中；摘要跨 codec 也相同，印证 codec 只是存储选择。
- **RSS 结论（新）**：压缩 codec 的进程 RSS **更低**而不是更高（B1 250.6 → 230.0/234.4
  MiB，B2 395.3 → 346.6/338.3 MiB，中位数），受管内存不变；已验证路径的 RSS 不高于
  精确路径（B2 389.3 对 433.0 MiB）。
- **P2a 结论修订**：第一轮"zstd 优于 lz4"的依据（去重与合并两个解压点）随 P6b 消失。
  复测只剩合并一个解压点：B1 上 zstd 仍更快；B2 上 zstd 的中位数被一次 6.19 秒离群
  抬高，其预热与最快单次（3.23 / 3.315 秒）低于 lz4 的全部单次，但单次波动远大
  （极差 2.87 秒对 0.03 秒）。本机三样本不足以裁决 lz4/zstd，取舍留给目标机器；
  默认保持不压缩。
- P2a 的"峰值内存"验收项与 P6b 的"B1/B2 内容差分"验收项据此勾选；P6b 剩余未验收项
  只有"自动回退"的评审豁免。

### P2a/P6b 评审修正

- 基准的每个 warm-up 与测量组合改在独立 spawned process 中运行，使 `ru_maxrss`
  成为单次运行峰值；旧 codec/dedup 报告的 RSS 是共享进程高水位，已明确降级为不可比较，
  等待在干净提交上重测。
- 每次运行记录 CSF 文本、CSF Parquet、descriptor 和 header 的 SHA-256；exact 与
  verified-unique 的 B1/B2 登记只有文件长度、没有内容摘要，计划状态已改为待重测。
- `CsfGenerationEstimate` 补齐运行时已经返回的 `deduplication` 字段，入口注释不再声称
  默认执行 exact；不可达的自动回退要求保持未勾选，等待评审豁免或未来可达边界。
- 源码指纹不再跟随未跟踪符号链接，并拒绝 `git status` 警告、Git 探测失败及递归目录中
  的不可读对象，避免读取仓库外数据或把不完整源码误判为干净。

### P6b：去重快速路径的计量（2026-09-22，登记于 `docs/benchmarks/v2_disk_generation_dedup_20260922.md`）

- 干净树 `bbcd714`、8 线程、每组合预热 1 次测量 3 次：跳过精确去重链路后，B1
  13.747 → 9.287 秒（−32.4%），B2 4.832 → 2.971 秒（−38.5%）；节省全部落在去重阶段
  （B1 4.549 → 0.005 秒，B2 1.683 → 0.001 秒），生成/合并/还原三段不变。
- scratch 峰值减半（B1 2142.5 → 1073.3 MiB，B2 1001.9 → 506.1 MiB）；去重阶段的临时
  写入从 1069.8/496.0 MiB 降到 0.3/0.1 MiB（剩下的就是幸存位图本身）。
- 与 P2a 的 zstd segment 组合后，scratch 峰值降到 segment 量级：B1 12.0 MiB、B2
  4.5 MiB（相对 exact+none 分别减少 99.4% / 99.6%）——即 P2b 重述后的验收形态；代价是
  合并阶段的解压（B1 +5.4%、B2 +10.5% 端到端）。
- 每次运行两种策略的计数与发布文件长度一致；该版 harness 未记录内容摘要，因此 B1/B2
  的内容差分仍待新版 harness 重测，不能用文件长度代替内容相等。

### P6b：在已证明的内部路径跳过精确去重（2026-09-22）

- 磁盘生成默认走 `verified_unique`：内部生成器不会产出重复行（P6a 证明），因此幸存
  位图直接写满，**不再写根去重桶、也不再逐行比较**——每行因此少写两遍、少读两遍。
  仍然保留的是 merge 按发布顺序读取 segment 的一遍，那是发布本身需要的。
- 策略由构造决定而不是调用者选项：`RCSFS_DEDUPLICATION=exact` 只能请求"更慢、重新
  核对证明"的精确路径，方向是收紧。统计、估算与 CLI JSON 新增 `deduplication` 字段，
  因为两种零的含义不同：`verified_unique` 是构造为零，`exact` 是比较得出的零。
- 容量模型按策略计费：已验证路径不再预留根桶与递归桶（此前会为不存在的桶预留多达
  数倍的 scratch，从而拒绝本可以完成的作业）；assumption 列表写明所选策略。
- 差分：Rust 侧对同一 transcript 跑两条路径并逐字节比较 CSF 文本、descriptor 行、计数
  与块结构；重复 segment 的反例证明两条路径确实不同（精确路径删重复、已验证路径保留），
  从而说明快速路径的适用范围来自构造。Python 侧在注册 fixture 上做同样的逐字节差分，
  新版基准脚本在一次调用中同时跑两种策略并在四个发布产物的 SHA-256 不一致时拒绝写出
  报告；旧 B1/B2 报告在此改动前生成，需重测后才具备这项证据。
