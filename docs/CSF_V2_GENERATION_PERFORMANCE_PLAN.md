...
状态：P0a/P0b 基准闭环、P1、P5a 计数与规划、P1.5 容量预检已实施；P2–P6 待实施，P6a 需先于 P2 给出唯一性结论。基线：`7ad18b1`。制定日期：2026-09-21；最近修订：2026-09-21。
...
- [x] 将本地 B1/B2 原始聚合测量及后续目标机器测量登记到 `docs/benchmarks/`，不记录机器专属绝对路径或生成的完整 CSF。本地 B1/B2 与 B3 容量报告已登记；目标机器测量仍未做，仍是 B4 的前置条件。

完成条件（P0a，已满足）：同一 API/脚本能运行 B1/B2 并生成阶段报告；B0 的文本、V2 schema/行顺序和元数据回归通过；1/2/4/8 线程比较入口存在且不进入默认 pytest。

完成条件（P0b）：B1/B2 输入可按哈希重现；基准不把删除临时输出混入生成墙钟；同机预热和至少 3 次重复报告中位数与波动；本地和目标机器的原始报告可审计。物理设备 I/O 只在平台可用时报告，不在代码中虚构。本机部分已满足（见 [docs/benchmarks](benchmarks/v2_disk_generation_p0b_20260921.md)）；目标机器报告仍缺，结论不能外推到其它机器。
...
完成条件（已满足）：完整输入只运行计数与 planner，登记任务预计 CSF 数的 p50/p95/max；在可拆分任务上，最大预计工作量不超过规划目标的 2 倍。线程数和切分方式不改变逻辑结果；单独列出无法满足的不可拆分任务，不掩盖长尾。B3 上 1,455 个任务的 p50/p95/max 为 3,999,089 / 3,999,953 / 3,999,999，目标 4,000,000，无不可拆分长尾。
...
- [x] 基于 B1/B2 和目标机器样本分别报告 descriptor、CSF 文本、CSF Parquet、segment、根桶、递归桶及发布 staging 的实测压缩比；报告区间和样本条件，不用单个最优比例作硬保证。
...
完成条件（已满足）：B1/B2 估算与实测差异可解释（B2 上为实测的 1.01–1.62 倍，上界来自递归分桶假设）；B3 可在不生成 58 亿行的情况下生成容量、写放大、range 工作量及时间区间报告；空间不足测试在写入大型 segment 前失败；B4 只在安全余量和失败恢复策略明确后启动。目标机器样本仍未取得，B4 启动前必须补齐。
